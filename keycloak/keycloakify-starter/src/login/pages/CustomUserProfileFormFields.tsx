...
import {useEffect, useReducer, Fragment, ChangeEvent, useCallback} from "react";
...
import {FieldType, InputField} from "../../../../../webapp/react/src/components/form/input-field/InputField.tsx";
import {useForm} from "react-hook-form";

export default function CustomUserProfileFormFields(props: UserProfileFormFieldsProps<KcContext, I18n>) {
    ...
    const defaultValues = formFieldStates
        // .reduce((values, state) => {values[] = ""}, {});
        .reduce(
            (values: any, state) => {
                const displayName = advancedMsgStr(state.attribute.displayName ?? "");
                if (displayName) {
                    values[displayName] = "";
                }
                return values;
    ...
    const getInputType = useCallback((attribute: Attribute) => {
                    ...
                    <Fragment key={attribute.name}>
                        ...
                        >
                            ...
                            <div className={kcClsx("kcInputWrapperClass")}>
                                ...
                                <InputField className="pt-3" control={control}
                                            name={advancedMsgStr(attribute.displayName ?? "")}
                                            inputId={advancedMsgStr(attribute.displayName ?? "")}
                                            required={attribute.required}
                                            inputProps={{name: attribute.name}}
                                            onChange={event =>
                                                dispatchFormAction({
                                                    action: "update",
                                                    name: attribute.name,
                                            ...
                                            error={
                                                displayableErrors && displayableErrors.length
                                                && displayableErrors[0].errorMessageStr
                                                || undefined
                                            }
                                            type={getInputType(attribute)}
                                            passwordMeter={attribute.name === "password"}
                                />
                                {/*<InputFieldByType*/}
                                {/*    attribute={attribute}*/}
                                {/*    valueOrValues={valueOrValues}*/}
